"""Built-in skill: full-text search across knowledge files in the current workspace."""
...
def run(input_data: dict, context: dict) -> dict:
    ...
    if not query:
        raise ValueError("search_workspace_knowledge: 'query' is required")
    ...
    try:
        ...
        row = con.execute(
            "SELECT w.name FROM swarms s JOIN workspaces w ON w.id = s.workspace_id WHERE s.id = ?",
            (swarm_id,),
        ).fetchone()
        con.close()
        if row:
    ...
    ws_path = os.path.join(data_dir, "workspaces", ws_name)
    candidates: list[dict] = []

    _collect(os.path.join(ws_path, "knowledge"), tokens, "workspace", candidates)
